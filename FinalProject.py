...
# Create save event
# save the current payment record.
def btn_save():
    ...
    if saveError == True:
        ...
        if saveError == True:
            ...
            FileWriter(myFile)
            FileWriter(f"BillPayment_Log-{dt.datetime.now():%Y%m%d}" + ".LOG")
            messagebox.showinfo("Save", "Payment saved successfully!")
            PrintReceipt()

            btn_clear()

# validate all data entry fields for completeness and free from errors
def Validate():
    vErr = False
...
# calculate change if cash method is selected based on payment amount and cash tendered.
...
# payment amount on change event module
...
# Create report window with printer report
def btn_report():
...
# Changes state of fields appropriately lock and disable according to the respective selection.
...
#Setup and initialize main windows GUI items.
...
#prevent main windows from accidental closing
root.protocol("WM_DELETE_WINDOW", disable_event)

#wait for an event to action
root.mainloop()
